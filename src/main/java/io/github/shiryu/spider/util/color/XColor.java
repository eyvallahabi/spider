package io.github.shiryu.spider.util.color;
...
public final class XColor {
  ...
  /**
   * the kyori enabled or not.
   */
  private static final AtomicBoolean kyoriEnabled = new AtomicBoolean(true);
  ...
  public XColor(@NotNull final String name, final char ch, final boolean color, final boolean reset,
    ...
    this.blue = blue;
    this.hexCode = null;
    if (ServerVersion.isServerVersionAtOrAbove(ServerVersion.V1_16) || !name.equalsIgnoreCase("Hex")) {
  ...
  /**
   * obtains the kyori enabled.
   *
   * @return kyori enabled.
   */
  public static boolean isKyoriEnabled() {
    return XColor.kyoriEnabled.get();
  }

  /**
   * sets the kyori enabled.
   *
   * @param status the status t oset.
   */
  public static void setKyoriEnabled(final boolean status) {
    XColor.kyoriEnabled.set(status);
  }
